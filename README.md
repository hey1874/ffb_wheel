...
📖 **详细接线(含 5V 电平陷阱、CAN 共地/终端、MCP23017 地址/RESET、供电与地):见 [`docs/WIRING.md`](docs/WIRING.md)。**
...
| 标志 | 默认 | 说明 |
|---|---|---|
| `MAX_NM` | `4.0` | FFB 满量程(±32767)对应的**输出(盘端)力矩**(Nm)。首次上机先验一次,见[已知问题](#已知问题) |
| `WHEEL_MAX_TURNS` | `1.5` | 方向盘 ±圈数(单边)。1.5 = ±540° = **1080° 总行程**;`1.25`→900°、`2.0`→1440°、`1.0`→720° |
| `ENDSTOP_NM` | `MAX_NM` | 软限位墙峰值力矩(盘端 Nm,实际封顶到 `MAX_NM`);`0` 禁用 |
| `ENDSTOP_RANGE_DEG` | `10.0` | 墙从 0 爬到满所需的超程角度(越大越"肉") |
...
### 行程与软限位(main.c,core0)
- **行程**由 `WHEEL_MAX_TURNS` 决定(单边圈数)。它只是"电机圈数 → 轴/弹簧度量"的刻度,**本身不挡你**。
- **软限位墙**(bump-stop):转过 ±`WHEEL_MAX_TURNS` 后,固件叠加一个回中力,在 `ENDSTOP_RANGE_DEG` 超程内从 0 爬到满 `ENDSTOP_NM`,模拟齿条到底。加在 FFB 引擎输出之上(不受游戏 device gain 影响),仅在电机健康时施加。
- **撞墙电流**:墙与 FFB 共用 `MAX_NM` 上限,峰值**不超过满力度 FFB**;有 `ENDSTOP_RANGE_DEG` 斜坡渐进。风险在于**长时间顶住 = 持续满电流发热**——务必在 ODrive 侧设好 `motor.config.current_lim` 作硬上限(墙超出即饱和,不会失控);想更保守就降 `ENDSTOP_NM` 到电机连续额定或加大 `ENDSTOP_RANGE_DEG`。
